...
- [ ] **T4.1** Dashboard pós-login: visão geral (total de decisões, últimas registradas)
- [ ] **T4.2** Filtrar "minhas decisões" na consulta (por `magistradoHash` do logado)
- [ ] **T4.3** Exibir status da assinatura na listagem de consulta (assinado ✓ / pendente ⚠)
- [ ] **T4.4** Botão "Arquivar" na consulta para decisões Publicadas/Retificadas
- [ ] **T4.5** `DecisaoService.arquivar()` — chamar endpoint `POST /api/decisoes/:hash/arquivar`
- [ ] **T4.6** Backend: endpoint `POST /api/decisoes/:hash/arquivar` → `contract.arquivarDecisao()`
- [ ] **T4.7** Sidebar: link para "Minhas Decisões" e "Configurações"
